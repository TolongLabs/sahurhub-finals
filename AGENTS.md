...
**SahurHub** — our Qwen Brainrot Hackathon entry: a deliberately useless product that nonetheless functions, powered centrally by a Qwen model.

Online Prelims run **14–19 July 2026**; the sole prelim deliverable is a **1080×1080 poster** (submitted by 19 July, 23:59 MYT) and community votes decide the shortlist. Shortlisted teams rebuild the prototype **on-site in 2 hours** at the Physical Final (25 July) — so everything built this week must be re-buildable from scratch, fast. Rules source of truth: `docs/project-requirements.md`. Product scope: `docs/prd.md`. Technical decisions: `docs/trd.md`.

---

## Architecture

See `docs/trd.md` (canonical). Do not create `docs/architecture.md`.

### Repo layout

```
docs/          PRD, TRD, official rules extract, workflow docs (roles/plan/progress/test/decisions — local-only), task-list
_research/     local research scratch (gitignored; deliverables live in docs/)
.agents/       committed, tool-agnostic skills (hackathon-* set; source of truth)
.claude/       Claude Code role agents, local settings, skills/ symlinks into .agents/skills/
.codex/        native Codex role agents (mirror of .claude/agents)
```

Source layout: TBD once the product concept locks (PRD §1).

---

## Tech Stack

**TBD — blocked on the product concept** (see `docs/trd.md` §3). Locked so far:

- **Bun** is the JS package manager / script runner (`bun install`, `bun run`, `bunx`).
- A **Qwen model** (exact model pinned in `docs/trd.md` §2) does the central work, accessed via Qwen Cloud or ModelScope.
- Every choice biases toward the 2-hour on-site rebuild: minimal deps, no long setup.
...
- **Types:** No `any`; prefer `unknown` + narrowing; validate at boundaries.
- **Formatting:** biome + prettier are authoritative — single quotes, no semicolons, 120-char lines, 2-space indent.
...
- **Formatting:** Always format text headings, subheadings, bullet-point headings, table-headings in TitleCase.
- **Conciseness:** Avoid clumping overly descriptive proses in one section, break down texts into smaller portions using elements like bullet points, tables or others.
...
- **Workflow visibility:** `hybrid` — the workflow inputs (`AGENTS.md`, `CLAUDE.md`, `.claude/agents/`, `.codex/`) are committed and shared; the generated pm-workflow artifacts (`docs/roles.md`, `docs/plan.md`, `docs/progress.md`, `docs/test.md`, `docs/decisions.md`, `docs/.pm-handoff.md`, `.claude/settings.local.json`) are **local-only** (listed in `.git/info/exclude`, never committed). Each contributor runs `/pm-workflow` on their own machine to scaffold them. Cross-team task coordination happens in `docs/task-list.md` (committed).
...
- **No secrets in repo.** `.env.example` committed, `.env` gitignored. Qwen API keys live in `.env.local`, never committed.
...
- **Do not** violate the hackathon Content Rules (`docs/project-requirements.md` §5) in any asset, output, or demo — a breach disqualifies the entry.
- **Do not** alter, obscure, or restyle the official Qwen logo in poster assets — it must appear unaltered.
- **Do not** miss the poster deadline: 19 July 2026, 23:59 MYT. Late = not shortlisted, no exceptions.
...
5. **Gate 2** — PM relays the verdict. Reject → back to PG. Approve → PM proposes a Conventional Commit message and **ships per this project's Gate 2 mode** (see Working Conventions / `docs/roles.md`): for substantial changes, open a PR and self-merge (small fixes commit straight to the target branch), deleting the branch on merge. It never pushes or merges without honoring that mode and any required human authorization.
...
Committed under `.agents/skills/` (source of truth; `.claude/skills/` symlinks into it). **Invoke them proactively when their situation arises — do not wait for an explicit request.** Each skill's frontmatter description carries its full trigger conditions; this table is the quick routing map:

| Situation                                                          | Skill                        |
| ------------------------------------------------------------------ | ---------------------------- |
| Product concept (PRD §1) still TBD or being rethought              | `hackathon-idea-generator`   |
| Candidate concepts exist, need ranking before Gate 1               | `hackathon-idea-scoring`     |
| Plan/feature list growing beyond the 2-hour on-site rebuild budget | `hackathon-scope-cutter`     |
| Concept locked; directing the poster or shaping the demo           | `hackathon-wow-detector`     |
| Preparing the finals presentation or rebuild-ending demo           | `hackathon-demo-script`      |
| Poster drafted or demo scripted — harden before submission/finals  | `hackathon-judge-simulator`  |
| Need winning patterns, demo psychology, MVP strategy references    | `hackathon-shared-resources` |
...
[Conventional Commits](https://www.conventionalcommits.org/): `<type>[scope]: <description>` — single imperative sentence, no trailing period. Allowed types: `feat`, `fix`, `refactor`, `docs`, `test`, `chore`, `style`, `perf`. The PM proposes the message at Gate 2; the human authorizes the commit. Enforced locally by commitlint (husky `commit-msg` hook).
